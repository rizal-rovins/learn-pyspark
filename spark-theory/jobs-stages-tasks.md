...
*   **Rule of Thumb:** 1 Action = 1 Job. (If you have 5 `write` commands in your script, you will have 5 Jobs).

## 2. The Stage ( Minimum 1 Stage. 1 Wide Dependency Transformation -> +1 Stage)
Spark cannot perform the entire Job in one go. It divides the Job into **Stages**. A Stage is a group of tasks that can run without needing data to move across the network.
...
## 3. The Task ( N Partitions -> N Tasks in the Stage)
...
## Putting it Together: The Life of a Query
...
| Metric | Count (Default) | Controlling Factor |
| :--- | :--- | :--- |
| **Jobs** | 1 | Triggered by Action (e.g., `write`). (+1 if `inferSchema=true`) |
| **Stages** | 2 | Wide Dependency (`groupBy` shuffle) |
| **Stage 1 Tasks** | ~8,192 | `spark.sql.files.maxPartitionBytes` (128 MB default) |
| **Stage 2 Tasks** | 200 | `spark.sql.shuffle.partitions` (200 default) |
